...
export class CodelensProvider implements vscode.CodeLensProvider {
	...
	public install(context: vscode.ExtensionContext) {
		...
		context.subscriptions.push(vscode.languages.registerCodeLensProvider(selector, this));
	...
	private installBuildTagBooleanCfg(context: vscode.ExtensionContext, cfgname: TSettings, commands: string[]) {
	...
	public provideCodeLenses(
		...
		if (!settings.enableCodeLens) { return []; }
		...
		const range1 = document.getWordRangeAtPosition(position1);

		if (iszig) {
			let moduleConfigFile = findModDefFile(document.fileName, 'build.zig');
			if (moduleConfigFile && range1) {
